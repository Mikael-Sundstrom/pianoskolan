...
var pianosounds = [];
for (var i=24; i<=108; i++) pianosounds[i] = null;

function pianosoundsrc(n)
{
 var prefix = 'http://www.apronus.com/static/grandpiano/';
...
function preloadPianoSound(context,whichsound)
{
 var request = new XMLHttpRequest();
 request.open("GET", pianosoundsrc(whichsound), true);
...
function preloadpianosounds(context,a,b)
{
 if (a == null) a = 24; //21;
 if (b == null) b = 108; //109;
 for (var n=a; n<=b; n++) preloadPianoSound(context,n);
}

function preloadpianooctaves(context,a,b)
{
 if (a == null) a = 3;
 if (b == null) b = 4;
 preloadpianosounds(context,(a+1)*12,(b+2)*12);
}

function preloadoctaves(a,b) { preloadpianooctaves(audiocontext,a,b); }
...
function playpianosound(context,n,delayInSeconds)
{
 ...
 {
  var ms = delayInSeconds * 1000;
  var play = "(new Audio(pianosoundsrc("+n+"))).play()";
  setTimeout(play,ms);
  console.log(err);
  console.log(play);
  return;
...
function soundbutton_class()
{
 ...
 text += ' margin: 1px; ';
 text += '}\n';
 text += '.soundbutton:hover { outline: 1px solid gray; }\n';
 text += '.soundbutton:active { transform: translateY(2px); }\n';
 style.appendChild(document.createTextNode(text));
 return style;
}

function soundbutton(n)
{
 var button = document.createElement('button');
 button.setAttribute('onmousedown', "playaudio("+n+")");
 button.innerHTML = soundname(n);
...
function soundbuttonsrow(a,b)
{
 var tr = document.createElement('tr');
 for (var i=a; i<=b; i++)
 {
  var td = document.createElement('td');
  td.appendChild(soundbutton(i));
  tr.appendChild(td);
 }
 return tr;
}

function soundbuttonstable()
{
 var table = document.createElement('table');
 table.className = 'soundtable';
 for (x=6; x>=1; x--)
 {
  table.appendChild(soundbuttonsrow(x*12+12,x*12+12+24));
 }
 return table;
}

function appendsoundbuttonstable()
{
 document.head.appendChild( soundbutton_class() );
 document.body.appendChild( soundbuttonstable() );
 preloadsounds();
}
...
function onrename(id)
{
 ...
 var newname = prompt('A new name for this button:',button.innerHTML);
 if (newname)
 {
...
function playbackbutton(id)
{
 ...
 button.innerHTML = (duration / 1000).toFixed(1) + ' secs playback';
...
function memorybutton(rechord)
{
 ...
 if (g_touchscreen == false)
 {
  ...
  editor.appendChild(keysensor(id));
 }
 else
 {
...
function recordpanel()
{
 ...
 rp.style.padding = '0';
 rp.appendChild(box);
 rp.appendChild(document.createTextNode('RECORD'));
...
function octavespanel(a,b)
{
 ...
 panel.setAttribute('onclick','this.style.display="none";');
...
function openpopup()
{
 ...
 var width = pianokeyboardpanel.clientWidth;
 var height = pianokeyboardpanel.clientHeight + 64;
 var popup = window.open('','','width='+width+',height='+height);
 var keys = "<script>document.body.onkeydown = keydown; document.body.onkeyup = keyup;</script>";
 var octsel = "<script>octaveselector.selectedIndex="+octaveselector.selectedIndex+";</script>";
 var melo = "<script>melochords.selectedIndex="+melochords.selectedIndex+";</script>";
 var adv = '<div id="advanced" style="display:none">' + advanced.innerHTML + '</div>';
 var donate = '<script>donatebutton.style.display = "inline";</script>';
 var preload = '<script>reloadsounds();</script>';
 var doc = "<!DOCTYPE html><html>";
 doc += "<head><title>PIANO</title>"+document.head.innerHTML+"</head><body style='margin-bottom:2em'>"+keys+html+octsel+melo+adv+donate+preload+"</body></html>";
...
function reloadsounds()
{
 ...
 {
  var key = document.getElementById("klawisz"+n);
  if (key) preloadPianoSound(audiocontext,n);
 }
}
...
function panel(a,b)
{
 ...
 se.setAttribute('onchange','playsfromga();');
...
function advancedpanel(a,b)
{
 ...
 chordpanel.appendChild(document.createTextNode('CHORD'));

 var memorybuttons = document.createElement('div');
 memorybuttons.id = 'memorybuttons';
 //memorybuttons.style.marginRight = '10em';
 ...
 adv.appendChild(chordpanel);
 adv.appendChild(recordpanel());
 adv.appendChild(savechordsform());
 adv.appendChild(loadchordsbutton());
 adv.appendChild(popupbutton());
...
function savechordsform()
{
 ...
 form.action = '//www.apronus.com/music/download.php';
 form.method = 'post';
 form.target = '_blank';
...
function logload()
{
 //var text = encodeURIComponent( memorybuttons.innerHTML );
 //(new Image).src = 'http://www.apronus.com/music/logload.php?t=' + text;
 galog("load","load");
}
